...
const ShareProduct = ({ product }) => {
  ...
  const shareOnWhatsApp = () => {
    const whatsappUrl = `https://wa.me/?text=${encodeURIComponent(productText)}`;
  ...
  return (
    <div className="border-t border-white/10 pt-6 mt-6">
      <div className="flex items-center space-x-3 mb-4">
        <Share2 size={18} className="text-[#E10600]" />
        <span className="text-sm font-bold">SHARE THIS PRODUCT</span>
      </div>
      <div className="flex items-center space-x-3">
        ...
        <motion.button
          ...
          className={`w-10 h-10 ${copied ? 'bg-green-500' : 'bg-gradient-to-br from-[#833AB4] via-[#FD1D1D] to-[#F77737]'} rounded-full flex items-center justify-center hover:opacity-80 transition-opacity`}
          whileHover={{ scale: 1.1 }}
          whileTap={{ scale: 0.95 }}
        ...
        <motion.button
          onClick={handleCopyUrl}
          className={`w-10 h-10 ${copied ? 'bg-green-500' : 'bg-white/10'} rounded-full flex items-center justify-center hover:bg-white/20 transition-colors`}
          whileHover={{ scale: 1.1 }}
          whileTap={{ scale: 0.95 }}
          title="Copy link"
